...
class PolymarketGamma:
    ...
    def fetch_comments(
        ...
        Args:
            parent_entity_id: The entity ID to fetch comments for (numeric market/event ID)
            entity_type: Entity type (e.g., "market", "event")
        ...
        def _fetch():
            ...
            resp = requests.get(
                f"{self.BASE_URL}/comments", params=params, timeout=self.timeout
            )
            resp.raise_for_status()
            data = resp.json()
            return data if isinstance(data, list) else []
    ...
    def fetch_profile(self, address: str) -> Dict:
        """
        Fetch a public user profile from the Gamma API.
        ...
        Note:
            This endpoint requires authentication (401 without it).
            May need API key or different access method.
        """
        ...
        def _fetch():
            ...
            if resp.status_code == 401:
                raise ExchangeError(
                    ...
                    "Provide API credentials to access profiles."
                )
            resp.raise_for_status()
            return resp.json()
