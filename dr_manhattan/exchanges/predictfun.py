...
class PredictFun(Exchange):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        # Market ID to decimal precision, used when complementing Yes prices to No.
    ...
    def _parse_market(self, data: Dict[str, Any]) -> Market:
        ...
        return Market(
            ...
            question=question,
            outcomes=outcomes,
            close_time=self._parse_datetime(data.get("endsAt") or data.get("closedAt")),
            volume=volume,
            liquidity=liquidity,
            prices=prices,
            metadata=metadata,
    ...
    def _is_market_closed(status: str, trading_status: str) -> bool:
        """Return true when the market is not open for new spread orders."""
        status_upper = status.upper()
        trading_upper = trading_status.upper()

        if status_upper in {"RESOLVED", "REMOVED", "PAUSED"}:
            return True
